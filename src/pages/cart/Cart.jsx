...
function Cart() {
  ...
  useEffect(() => {
    ...
    cartItems.forEach((cartItem) => {
      temp = temp + parseFloat(cartItem.price) * cartItem.quantity;
    });
    setTotalAmount(temp);
    // console.log(temp);
  }, [cartItems]);
        ...
        <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0 ">
          <div className="rounded-lg md:w-2/3 ">
            {cartItems.map((item, index) => {
              const { id,quantity, title, category, imageURL, price, description } =
                item;
              return (
                <div
                  key={index}
                  ...
                  <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                    <div className="mt-5 sm:mt-0">
                      ...
                      <div className="flex items-center mt-2">
                        <button onClick={() => decreaseItemQuantity(item)} className="px-2 py-1 bg-gray-400 rounded-md">-</button>
                        <p className="mx-2">{quantity}</p>
                        <button onClick={() => increaseItemQuantity(item)} className="px-2 py-1 bg-gray-400 rounded-md">+</button>
                      </div>
                    </div>
